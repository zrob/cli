		...
		When("the stack exists", func() {
			...
			When("the stack exists and the --guid flag is passed", func() {
				It("prints nothing but the guid", func() {
					...
					// CAPI GUID regex
					matcher := Say(`^[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}$`)
					succ, _ := matcher.Match(BufferWithBytes([]byte("f17a7885-675a-4d1f-84f4-104401a33192")))
					if succ {
						fmt.Printf("WOOOOHHHHHOOOOOOOO\n\n\n")
					} else {
						fmt.Printf("AWWWWWWWWWWWWWW\n")
					}
